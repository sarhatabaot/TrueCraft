...
namespace fNbt
{
	...
	{
		[NotNull] private readonly List<NbtTag> tags;
		...
		///  If givenListType is Unknown, ListType will be infered from the first tag added to this NbtList.
		/// </param>
		/// <exception cref="ArgumentOutOfRangeException"> <paramref name="givenListType" /> is not a recognized tag type. </exception>
		...
		public NbtTagType ListType
		{
			...
			set
			{
				if (!Enum.IsDefined(typeof(NbtTagType), value)) throw new ArgumentOutOfRangeException("value");
		...
		public override NbtTag this[int tagIndex]
		{
			...
			set
			{
				...
				else if (value.TagType != listType) throw new ArgumentException("Items must be of type " + listType);
				tags[tagIndex] = value;
				value.Parent = this;
			}
		}
		...
		public T[] ToArray<T>() where T : NbtTag
		{
			...
			for (var i = 0; i < result.Length; i++) result[i] = (T) tags[i];
			return result;
		}


		/// <summary>
		///  Returns a String that represents the current NbtList object and its contents.
		///  Format: TAG_List("Name"): { ...contents... }
		/// </summary>
		...
		internal override bool ReadTag(NbtBinaryReader readStream)
		{
			...
			if (length < 0) throw new NbtFormatException("Negative count given in TAG_List");
		...
		internal override void WriteData(NbtBinaryWriter writeStream)
		{
			...
			foreach (var tag in tags) tag.WriteData(writeStream);
		}
		...
		public void Clear()
		{
			for (var i = 0; i < tags.Count; i++) tags[i].Parent = null;
			tags.Clear();
		}
		...
		///  the number of tags in this NbtList is greater than the available space from arrayIndex to the end of the destination
		///  array;
		...
		void IList.Remove(object value)
		{
			...
			if (tags.Remove(val)) val.Parent = null;
		}
