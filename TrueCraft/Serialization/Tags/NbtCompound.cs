...
namespace fNbt
{
	...
	public sealed class NbtCompound : NbtTag, ICollection<NbtTag>, ICollection
	{
		...
		public NbtCompound([CanBeNull] string tagName) => Name = tagName;
		...
		public NbtTag[] ToArray()
		{
			...
			var i = 0;
			foreach (var tag in tags.Values) array[i++] = tag;
			return array;
		}
		...
		public string[] ToNameArray()
		{
			...
			var i = 0;
			foreach (var tag in tags.Values) array[i++] = tag.Name;
			return array;
		}
		...
		/// <summary>
		///  Returns a String that represents the current NbtCompound object and its contents.
		///  Format: TAG_Compound("Name"): { ...contents... }
		/// </summary>
		...
		internal override bool ReadTag(NbtBinaryReader readStream)
		{
			...
			while (true)
			{
				...
				if (newTag.ReadTag(readStream)) tags.Add(newTag.Name, newTag);
			}
		}
		...
		{
			foreach (var tag in tags.Values) tag.WriteTag(writeStream, true);
		...
		public void Clear()
		{
			foreach (var tag in tags.Values) tag.Parent = null;
			tags.Clear();
		}
