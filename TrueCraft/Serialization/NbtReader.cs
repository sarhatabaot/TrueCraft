...
namespace fNbt
{
	...
	public class NbtReader
	{
		...
		private readonly Stack<NbtReaderNode> nodes = new Stack<NbtReaderNode>();
		...
		///  <c>RootTag</c> is at depth 0, its descendant tags are 1, etc.
		/// </summary>
		...
		///  No further reading can be done from this instance if a parse error occured.
		/// </summary>
		...
		{
			if (!atValue) throw new NbtFormatException(NoValueToReadError);
			switch (TagType)
			{
				case NbtTagType.Byte:
					reader.ReadByte();
		...
		///  In other words, reads until parent tag's subling.
		/// </summary>
