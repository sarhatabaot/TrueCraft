...
namespace TrueCraft.Tests.Serialization
{
	...
	public class NbtFileTests
	{
		...
		public void NbtFileTestTearDown()
		{
			...
			{
				foreach (var file in Directory.GetFiles("TestTemp")) File.Delete(file);
		...
		private void AssertNbtBigFile(NbtFile file)
		{
			...
			for (var n = 0; n < 1000; n++) Assert.AreEqual((n * n * 255 + n * 7) % 100, ((NbtByteArray) node)[n]);
			...
			for (var i = 0; i < 10; i++) Assert.AreEqual(intArrayTag.Value[i], rand.Next());
		}
		...
		{
			var fileBytes = File.ReadAllBytes("TestFiles/bigtest.nbt");
		...
		{
			var file = new NbtFile();
			var length = file.LoadFromFile("TestFiles/bigtest.nbt.gz");
			AssertNbtBigFile(file);
		...
		{
			var file = new NbtFile();
			var length = file.LoadFromFile("TestFiles/bigtest.nbt");
			AssertNbtBigFile(file);
			Assert.AreEqual(length, new FileInfo("TestFiles/bigtest.nbt").Length);
		}
		...
		{
			var loadedFile = new NbtFile("TestFiles/bigtest.nbt");
			var bytesWritten = loadedFile.SaveToFile("TestTemp/bigtest.nbt.gz", NbtCompression.GZip);
			var bytesRead = loadedFile.LoadFromFile("TestTemp/bigtest.nbt.gz", NbtCompression.AutoDetect, null);
		...
		public void ReloadNonSeekableStream()
		{
			...
			using (var nss = new NonSeekableStream(ms))
			{
				...
				ms.Position = 0;
				var bytesRead = loadedFile.LoadFromStream(nss, NbtCompression.None, null);
				Assert.AreEqual(bytesWritten, bytesRead);
				AssertNbtBigFile(loadedFile);
			}
		}
		...
		{
			var loadedFile = new NbtFile("TestFiles/bigtest.nbt");
			var bytesWritten = loadedFile.SaveToFile("TestTemp/bigtest.nbt", NbtCompression.None);
			var bytesRead = loadedFile.LoadFromFile("TestTemp/bigtest.nbt", NbtCompression.AutoDetect, null);
			Assert.AreEqual(bytesWritten, bytesRead);
			AssertNbtBigFile(loadedFile);
		}
		...
		public void TestNbtSmallFileSavingUncompressedStream()
		{
			...
			var testFileStream = File.OpenRead("TestFiles/test.nbt");
