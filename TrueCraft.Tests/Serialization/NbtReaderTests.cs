...
namespace fNbt.Test
{
	...
	{
		private static Stream MakeTest()
		{
		...
		public void NestedListTest()
		{
			...
			using (var ms = new MemoryStream(testData))
			{
				...
				while (reader.ReadToFollowing()) Console.WriteLine(reader.ToString(true));
			}
		}
		...
		{
			using (var fs = File.OpenRead("TestFiles/bigtest.nbt"))
			{
		...
		{
			var reader = new NbtReader(MakeTest());
